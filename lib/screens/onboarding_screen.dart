...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  @override
  Widget build(BuildContext context)=>Scaffold(body:SafeArea(child:Center(child:ConstrainedBox(constraints: BoxConstraints(maxWidth:520),
    child:ListView(padding: EdgeInsets.all(28),children:[
      Row(children:[Image.asset('assets/icon-512.png',width:34,height:34), SizedBox(width:10), Text('Josh Tracker',style:TextStyle(fontWeight:FontWeight.w600)), Spacer(),Text('${_step+1} / 3',style: TextStyle(color:C.label2))]),
       SizedBox(height:46),
      ...
      if(_step==0)...[
        TextField(controller:_name,maxLength:30,decoration: InputDecoration(labelText:'What should we call you?')),gap,
        TextField(controller:_goal,maxLength:100,maxLines:2,decoration: InputDecoration(labelText:'What are you working towards?')),
      ],
      if(_step==1)...[
        if(store.habits.isNotEmpty)Padding(padding: EdgeInsets.only(bottom:18),child:Text('Your ${store.habits.length} existing habits and history are kept. Add anything you want below.',style: TextStyle(color:C.label2,height:1.5))),
        for(var i=0;i<starterHabits.length;i++)Padding(padding: EdgeInsets.only(bottom:10),child:Panel(padding:4,child:CheckboxListTile(
      ...
      if(_step==2)...[
        Panel(child:Column(children:[
          SwitchListTile.adaptive(contentPadding:EdgeInsets.zero,value:_enable,title: Text('Enable reminders'),subtitle: Text('You stay in control'),onChanged:(v)=>setState(()=>_enable=v)),
          _time('Quiet hours begin',_quietStart,(v)=>setState(()=>_quietStart=v)),
          _time('Quiet hours end',_quietEnd,(v)=>setState(()=>_quietEnd=v)),
        ])),gap,
         Text('Choose each habit’s reminder time in Settings. Notifications check progress saved on this phone. Android may deliver them a little later to save battery.',style:TextStyle(color:C.label2,fontSize:13,height:1.6)),
      ],
       SizedBox(height:32),ActionButton(_busy?'Setting things up…':_step==2?'Begin your next chapter':'Continue',_busy?null:()async{
      ...
      if(_step>0)TextButton(onPressed:_busy?null:()=>setState(()=>_step--),child: Text('Back')),
    ])))));
