...
class PageTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text(title,style: TextStyle(fontSize:34,fontWeight:FontWeight.w700,letterSpacing:-1.2)),
       SizedBox(height:7),Text(subtitle,style: TextStyle(color:C.label2,fontSize:14,height:1.5)),
    ]),
  );
}
class SectionLabel extends StatelessWidget {
  ...
  final String label;
  @override
  Widget build(BuildContext context) => Padding(padding: EdgeInsets.fromLTRB(2,8,2,12),
    child:Text(label,style: TextStyle(fontSize:19,fontWeight:FontWeight.w600,letterSpacing:-0.4)));
}
class ActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(width:double.infinity,
    child:FilledButton(onPressed:onPressed,style:FilledButton.styleFrom(
      minimumSize: Size(48,52),backgroundColor:secondary?C.card2:C.label,
      foregroundColor:secondary?C.label:C.bg,
...
class ProgressRing extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(label:'Today progress ${(value*100).round()} percent',
    child:TweenAnimationBuilder<double>(tween:Tween(begin:0,end:value),
      duration:MediaQuery.of(context).disableAnimations?Duration.zero: Duration(milliseconds:650),
      curve:Curves.easeOutCubic,builder:(context,v,_)=>SizedBox(width:size,height:size,
        child:CustomPaint(painter:_Ring(v),child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[
          Text('${(v*100).round()}%',style: TextStyle(fontSize:38,fontWeight:FontWeight.w600,letterSpacing:-1.8)),
           SizedBox(height:3), Text('OF TODAY',style:TextStyle(color:C.label2,fontSize:10,letterSpacing:1.5)),
        ]))))));
}
class _Ring extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_Ring old) => true;
}
...
Future<void> logHabit(BuildContext context,Habit h) async {
  ...
  final earned=store.data.milestones.where((m)=>m.earned).map((m)=>m.id).toSet();
  void celebrate(){
    if(!context.mounted)return;
    final unlocked=store.data.milestones.where((m)=>m.earned&&!earned.contains(m.id)).toList();
    if(unlocked.isNotEmpty){message(context,'Milestone reached · ${unlocked.first.title}${unlocked.length>1?' +${unlocked.length-1} more':''}');}
    else if(before>0 && store.data.remaining(store.now).isEmpty){message(context,'All done. Take a moment to enjoy it.');}
  }
  if(h.kind!='quantity') {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child:SingleChildScrollView(padding:EdgeInsets.fromLTRB(24,8,24,24+MediaQuery.of(context).viewInsets.bottom),
      child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.stretch,children:[
        Text(h.name,style: TextStyle(fontSize:26,fontWeight:FontWeight.w600)),
         SizedBox(height:8),Text('Today’s total · goal ${number(h.target)} ${h.unit}',style: TextStyle(color:C.label2)),gap,
        TextField(controller:_text,autofocus:true,keyboardType: TextInputType.numberWithOptions(decimal:true),
        ...
        Wrap(spacing:10,children:[
          ...
          TextButton(onPressed:()=>_text.text=number(h.target),child: Text('Reach goal')),
        ]),gap,
